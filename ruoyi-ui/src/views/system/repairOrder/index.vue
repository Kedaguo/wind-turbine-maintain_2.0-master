  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="repairOrderList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label=" 发生时间" align="center" prop="rCreateTime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.rCreateTime, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="风机编号" prop="tId">
          <el-input v-model="form.tId" placeholder="请输入风机编号" />
        </el-form-item>
        <el-form-item label="任务编号" prop="taskId">
        ...
        <el-form-item label="派单状态" prop="rState">
          <el-select v-model="form.rState" placeholder="请选择派单状态">
            <el-option
              v-for="dict in dict.type.repair_order_state"
              :key="dict.value"
              :label="dict.label"
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加repairOrder";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const rId = row.rId || this.ids
      getRepairOrder(rId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改repairOrder";
      });
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除repairOrder编号为"' + rIds + '"的数据项？').then(function() {
        return delRepairOrder(rIds);
      }).then(() => {
        this.getList();
